...
def create_work_order_tool(db: Session, code: str, customer_name: str = None, priority: int = 3, due_date_days_from_now: int = 7) -> str:
...
def add_operation_tool(db: Session, work_order_code: str, machine_code: str, processing_minutes: int) -> str:
...
def recompute_schedule(db: Session) -> str:
    ...
    return f"New schedule computed (Run ID: {run.id}).
...
def get_schedule_summary_text(db: Session) -> str:
    ...
    summary = get_schedule_summary(run)
    return f"Latest Schedule Summary (Run ID: {run.id}, Algorithm: {run.algorithm}):
...
TOOLS = [
    ...
    {
        "name": "add_operation",
        "description": "Add a processing step to a work order. Must specify work order, machine, and duration.",
        "parameters": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                "work_order_code": {"type": "string"},
                "new_due_date": {"type": "string", "description": "ISO format date string"}
            },
    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                "work_order_code": {"type": "string"},
                "priority": {"type": "integer", "description": "1-5 scale"}
            },
